...
public class TenTruckInfoAdapter extends RecyclerView.Adapter<TenTruckInfoAdapter.InfoViewHolder> {
    ...
    private List<TenTruckLogs> tenTruckLogs=MainActivity.tenTruckLogsDatabase.tenTruckLogsDao().read();
    ...
    TenTruckInfoAdapter(Context context) { mInflater = LayoutInflater.from(context); }
    ...
    public void onBindViewHolder(InfoViewHolder holder, int position) {
      /* for (Logs logs1: logs)//for each loop
        ...
        if (tenTruckLogs != null) {
            TenTruckLogs logs1 = tenTruckLogs.get(position);
            String mdrivername = logs1.getMdrivername();
            String mregno = logs1.getMregno();
            String mstartime = logs1.getMstartime();
            String m1stbreak = logs1.getM1stbreak();
